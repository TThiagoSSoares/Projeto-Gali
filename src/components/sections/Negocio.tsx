...
import { ofertas, receitaRecorrente, contato, disclaimer } from "@/data/negocio";
  ...
  return (
    <section id="negocio" className="border-t border-panel-border px-4 py-20">
      <div className="mx-auto max-w-4xl">
        ...
        <Reveal
          delay={260}
          className="mt-6 rounded-xl border border-panel-border bg-panel p-5"
        >
          <p className="text-xs font-semibold uppercase tracking-wide text-accent-warm">
            Receita recorrente
          </p>
          <h3 className="mt-1 font-heading text-lg font-bold">
            {receitaRecorrente.titulo}
          </h3>
          <p className="mt-2 text-sm text-muted">{receitaRecorrente.texto}</p>
        </Reveal>
